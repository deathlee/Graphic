...
public class PlayerSpring implements MouseMotionListener, KeyListener {
	...
	public void draw(Graphics2D of) {
		...
		of.setStroke(new BasicStroke((float) 0.05));
		Circle circle=new Circle(new XYPoint(n.x, n.y+0.40), 0.20);
		circle.draw(of);
		of.setColor(Color.blue);
		of.setStroke(new BasicStroke((float) 0.05));
		of.draw(new Line2D.Double(n.x, n.y, n.x, n.y+0.2));
		of.setColor(Color.red);
		of.setStroke(new BasicStroke((float) 0.05));
		lRad.draw(of);
		of.setColor(Color.red);
		...
		rTib.draw(of);
		of.setColor(Color.blue);
		of.fill(new Rectangle2D.Double(Lum1.p2.x-0.20, Lum1.p2.y,0.40,0.53));
		lHum.draw(of);
		double l=1.0;
		double a=Math.PI/2;
		long time = System.currentTimeMillis()/2000;
		of.draw(new Line2D.Double(p.getX(),p.getY(),p.getX()+Math.sin(a*Math.sin(time)),p.getY()+Math.cos(a*Math.sin(time))));
	}
	/*public void draw(Graphics2D of) throws IOException {
		BufferedImage head = ImageIO.read(new File("/Users/zhuangli/Downloads/jerome.png"));
		of.drawImage(head,  (int)((n.x)*100-20), (int)((6-n.getY())*100)-35,40, 35,null);
		BufferedImage body = ImageIO.read(new File("/Users/zhuangli/Downloads/guike.png"));
		of.drawImage(body,  (int)(n.x*100-20),(int)((6-n.getY())*100),40, 60,null);
		of.setTransform(wall.trans);
		of.setColor(Color.black);
		of.setStroke(new BasicStroke(0.01f));
	    ...
	    of.draw(new Line2D.Double(n.getX()-3,6,n.getX(),n.getY()));
		lFem.draw(of);
		rFem.draw(of);
		Circle c1 = new Circle(lf, 0.1);
	    Circle c2 = new Circle(rf, 0.1);
	    c1.draw(of);
	    c2.draw(of);
	}*/
