...
public abstract class LineAlias {
	...
	public static void main(String[] args) throws InterruptedException {
		...
		Line line = new Line() {
			void line(BufferedImage buf, int x1, int y1, int x2, int y2, int rgb) {
				  ...
				  if ( Math.abs(dx) > Math.abs(dy) ) {
				    ...
				    System.out.println(xpxl1);
				    buf.setRGB(xpxl1, ypxl1,  RGB(rfpart(yend)*xgap,rgb));
				    buf.setRGB(xpxl1, ypxl1+1,RGB(fpart(yend)*xgap,rgb));
				    ...
				    buf.setRGB(xpxl2, ypxl2, RGB(rfpart(yend)*xgap,rgb));
				    buf.setRGB(xpxl2, ypxl2 + 1, RGB(fpart(yend) * xgap,rgb));
				    int x;
				    ...
				    buf.setRGB(xpxl1, ypxl1, RGB(rfpart(xend)*ygap,rgb));
				    buf.setRGB(xpxl1, ypxl1+1, RGB(fpart(xend)*ygap,rgb));
				    ...
				    buf.setRGB(xpxl2, ypxl2, RGB(rfpart(xend) * ygap,rgb));
				    buf.setRGB(xpxl2, ypxl2 + 1, RGB( fpart(xend) * ygap,rgb));
				    int y;
		...
		/*Line line = new Line(){
            void line(BufferedImage buf, int x0, int y0, int xn, int yn, int rgb) {
                ...
                //System.out.println("x0: "+x0+"; xn: "+xn+", y0:" +y0+", yn:"+yn);
                boolean steep = Math.abs(yn - y0) > Math.abs(xn - x0);
                if (steep){
                    int[] swap = swap(x0,y0);
                    x0 = swap[0];
                    y0 = swap[1];
                ...
                if (dx != 0){
                    ...
                    gradient = 0;
                int xend = round(x0);
                float yend = (float) y0 + (float)gradient*(xend - x0);
                //System.out.println(yend);
                float xgap = rfpart((float) (x0 +0.5));
                int xpxl1 = xend;
                int ypxl1 = ipart(yend);
                if(!steep){
                    plot(buf,xpxl1,ypxl1,rfpart(yend)*xgap,rgb);
                    plot(buf,xpxl1,ypxl1+1,fpart(yend)*xgap,rgb);
                }else{
                    plot(buf,ypxl1,   xpxl1, rfpart(yend)*xgap,rgb);
                    plot(buf,ypxl1+1, xpxl1,  fpart(yend)*xgap,rgb);
                }
                float intery = yend +  gradient; 
                //System.out.println("intery:"+yend+" + "+ gradient+" = "+intery);
                //second endpoint
                xend = round(xn);
                yend = (float)yn + (float)gradient * (xend - xn);
                xgap = fpart( (float) (xn +0.5));
                int xpxl2 = xend;
                int ypxl2 = ipart(yend);
                if(!steep){
                    plot(buf,xpxl2,ypxl2,rfpart(yend)*xgap,rgb);
                    plot(buf,xpxl2,ypxl2+1,fpart(yend) * xgap,rgb);                    
                }else{
                    plot(buf,ypxl2,   xpxl2, rfpart(yend) * xgap,rgb);
                    plot(buf,ypxl2+1, xpxl2,  fpart(yend) * xgap,rgb);                    
                }
                

                for (int x = xpxl1 +1; x < xpxl2;x++){
                    if(!steep){
                        plot(buf,x,ipart(intery), rfpart(intery),rgb);        
                        plot(buf,x,ipart(intery)+1,fpart(intery),rgb);
                    }else{
                        plot(buf,ipart (intery),x,rfpart(intery),rgb);
                        plot(buf,ipart (intery)+1,x, fpart(intery),rgb);                        
                    }
                    intery += gradient;
                }
	...
	static int RGB(double br,int rgb){
		if (br>1.0) {
			br=1.0;
		}
		
		Color color= new Color(rgb);
		int r=(int)((color.getRed()*br)+255*(1-br));
		int g=(int)((color.getGreen()*br)+255*(1-br));
		int b=(int)((color.getBlue()*br)+255*(1-br));
		System.out.println(r);
		System.out.println(g);
		System.out.println(b);
		Color reColor = new Color(r,g,b);
		//Color reColor = new Color(255, 2, (int)(color.getBlue()*br));
		return reColor.getRGB();
	}
	 public static void plot (BufferedImage buf, float x, float y, float c, int rgb){
	        //System.out.println("x: "+x+",y: "+y+", brightness: "+c +",rgb:"+rgb);
	        Color color = new Color(rgb);
	        int r = (int) Math.rint(color.getRed()*c + 255*(1-c));
	        int g = (int) Math.rint(color.getGreen()*c+ 255*(1-c));
	        int b = (int) Math.rint(color.getBlue()*c+ 255*(1-c));
	        int rgbNew = new Color(r, g, b).getRGB();
	        //System.out.println("newRgb:"+rgbNew);
	            //rgbNew[i] = (int) (rgb[i] * brightness + baseRgb[i] * (1 - brightness));
	        buf.setRGB(Math.round(x),Math.round(y),rgbNew);
	    }
	    ...
	    public static float fpart(float x){
	        if (x<0)
	            return (float) ((float) 1 - (x - Math.floor(x)));
	        //System.out.println("fpart of "+x+":"+ ((float) x - Math.floor(x)));
	        return (float) ((float) x - Math.floor(x));
	    }
	    public static float rfpart(float x){
	        //System.out.println("rfpart of "+x+":"+((float) 1- fpart(x)));
